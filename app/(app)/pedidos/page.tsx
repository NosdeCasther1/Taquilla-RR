...
import { Users, Bell, Check, RotateCcw, StickyNote } from "lucide-react";
...
type Filter = "PENDIENTE" | "ENTREGADO" | "TODOS";
...
function statusVariant(status: Order["status"]): "warning" | "success" | "secondary" {
...
export default function PedidosPage() {
  ...
  // Siempre poll de pendientes para alertas y título de pestaña
  ...
  async function setStatus(order: Order, status: "PENDIENTE" | "ENTREGADO") {
    markSeen(order.id);
    ...
    <div className="space-y-4">
      ...
      {!isLoading && orders?.length === 0 && (
        <p className="py-8 text-center text-sm text-muted-foreground">
          {filter === "PENDIENTE" ? "No hay pedidos pendientes." : "No hay pedidos aquí."}
        </p>
      )}
        ...
        {orders?.map((order) => {
          ...
          return (
            <Card
              key={order.id}
              className={cn(
                (order.status === "ENTREGADO" || order.status === "CANCELADO") && "opacity-70",
                order.status === "CANCELADO" && "border-dashed",
